...
export const DatasetFilterBar: React.FC<DatasetFilterBarProps> = ({
  ...
  return (
    <div className="bg-[#16171A] rounded-3xl border border-[#2A2B2F] p-5 mb-6 shadow-sm space-y-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
          ...
          {isFiltered && (
            <button
              ...
              className="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-bold text-rose-400 hover:text-rose-300 bg-rose-950/40 hover:bg-rose-950/70 rounded-xl border border-rose-800/40 transition-colors"
            >
          ...
          <div className="text-xs text-[#8E8F94] font-medium px-3 py-1.5 bg-[#1C1D21] border border-[#2A2B2F] rounded-xl">
            Showing <span className="text-[#D4AF37] font-bold">{filteredCount}</span> of {totalQuestions}
          </div>
        </div>
      </div>

      {/* Filter Row 1: Letter selection */}
      <div>
        <div className="text-xs font-bold text-[#8E8F94] uppercase tracking-wider mb-2 flex items-center gap-1.5">
          ...
          {letters.map((ltr) => (
            <button
              ...
              className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${
                filter.letter === ltr
                  ? 'bg-[#D4AF37] text-[#0F1012] shadow-xs scale-105 font-black'
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-3 border-t border-[#2A2B2F]">
        <div>
          <span className="text-xs font-bold text-[#8E8F94] uppercase tracking-wider block mb-1.5">
            Vocabulary Type:
          </span>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {[
              ...
              { id: 'single', label: '1 Syn/Ant (Direct)' },
              { id: 'multiple_both', label: '2 Syn/Ant ("Both A & B")' },
              { id: 'negative_not', label: '3+ Syn/Ant ("NOT a Syn/Ant")' }
            ].map((typ) => (
              <button
                key={typ.id}
